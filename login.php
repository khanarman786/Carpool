...
$logdone=false;
$logcredential=false;
if(empty($_POST['fname']) || empty($_POST['lname']) || empty($_POST['mno']) || empty($_POST['email']) || empty($_POST['password'])){
  $emptyerror = true;
  $logdone = false;
}
...
if(isset($_POST['logbtn'])){
  ...
  $password=$_POST['password'];
  $login="SELECT * FROM `user` WHERE `email`='$username' AND `password`='$password'";
  $result=mysqli_query($con,$login);
  $Userfname=mysqli_fetch_assoc($result);
  $userfnamefor=$Userfname['fname'];
  $useremail=$Userfname['email'];
  $userphone=$Userfname['mno'];
  $_SESSION['useremail']=$useremail;
  $_SESSION['userphone']=$userphone;
  $_SESSION['user']=$userfnamefor;
...
<script>function onlyNumber(evt) {
    ...
    <div class="modal-dialog text-center" role="document">
      <div class="modal-content text-center">
        ...
        <div class="modal-body text-center">
                ...
                <input type="text" maxlength="10" pattern="\d{10}" onkeypress="return onlyNumber(event)" name="mno" placeholder="Enter your mobile no" id="signup" class="input1" required autocomplete="off">
                <input type="email" name="email" placeholder="Enter your email" id="signup" class="input1" required autocomplete="off">
                <input type="text" name="password" placeholder="Enter your password" id="signup"class="input1" required autocomplete="off">
                <input type="text" name="cpassword" placeholder="Confirm your passsword"id="signup" class="input1"" required autocomplete="off">
        </div>
